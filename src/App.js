...
class App extends Component {
  ...
  componentWillMount() {
    ...
    let messagesRef = fire.database().ref('messages').orderByChild('channel').limitToLast(100).equalTo(this.props.channel);
  ...
  addMessage(e) {
    ...
    fire.database().ref('messages').push({ text: this.inputEl.value, channel: this.props.channel, user: this.props.user });
  ...
  handleUploadSuccess = (filename) => {
    ...
    fire.storage().ref('images').child(filename).getDownloadURL().then(imageURL => {
      fire.database().ref('messages').push({ imageURL, channel: this.props.channel, user: this.props.user });
    });
  };
  render() {
    return (
      <div>
        <nav className="navbar is-fixed-top" aria-label="main navigation">
          <div className="navbar-brand">
            ...
            <span className="navbar-item">{this.props.channel}</span>
          </div>
        </nav>
              ...
              <div className="usermessage" key={message.id}>
                ...
                }
                {
                  message.message.text ?
                    ...
                    <ClickableImage onLoad={this.handleImageLoaded.bind(this)} imageURL={message.message.imageURL}/>
                }
              </div>
            ))
          }
